...
namespace Studi.Proctoring.BackOffice_Api.Middlewares.Exceptions
{
    public class ExceptionMiddleware
    {
        ...
        public async Task Invoke(HttpContext httpContext)
        {
            ...
            catch (CannotBeArchivedAggregateException ex)
            {
                await HandleException(httpContext, ex, HttpStatusCode.Forbidden, string.Join(", ", ex.InnerExceptions.Select(iex => iex.Message)));
            }
            catch (GlobalSettingWrongValueTypeException ex)
            {
                await HandleException(httpContext, ex, HttpStatusCode.BadRequest, ex.Message);
            }
