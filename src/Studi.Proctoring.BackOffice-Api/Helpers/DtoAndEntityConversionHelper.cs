...
namespace Studi.Proctoring.BackOffice_Api.Helpers
{
    public class DtoAndEntityConversionProfile : Profile
    {
        ...
        public DtoAndEntityConversionProfile()
        {
            ...
            CreateMap<UserExamDetailInfosDto, UserExamDetailInfosVM>()
                .ForMember(userExam => userExam.UserPictureCheckImage, opt => opt.Ignore())
                .ForMember(userExam => userExam.UserIdentityCheckImage, opt => opt.Ignore());

            CreateMap<ProctoringImage, ProctoringImageDto>();
            CreateMap<ProctoringImageDto, ProctoringImage>();
            CreateMap<ProctoringImageForUserExamDto, ProctoringImageForUserExamVM>();
    ...
    public static class ConversionExtension
    {
        ...
        public static UserExamDetailInfosVM ToVM(this UserExamDetailInfosDto userExamDetailInfosDto)
        {
            if (userExamDetailInfosDto is null)
                return null;
            else
            {
                var userExamDetailInfosVm = _mapper.Map<UserExamDetailInfosVM>(userExamDetailInfosDto);

                if (!String.IsNullOrWhiteSpace(userExamDetailInfosDto.UserIdentityCheckImage))
                    userExamDetailInfosVm.UserIdentityCheckImage = File.ReadAllBytes(userExamDetailInfosDto.UserIdentityCheckImage);
                if (!String.IsNullOrWhiteSpace(userExamDetailInfosDto.UserPictureCheckImage))
                    userExamDetailInfosVm.UserPictureCheckImage = File.ReadAllBytes(userExamDetailInfosDto.UserPictureCheckImage);

                return userExamDetailInfosVm;
            }
        }
        ...
        public static ProctoringImageForUserExamVM ToVM(this ProctoringImageForUserExamDto proctoringImageForUserExamDto)
        {
            if (proctoringImageForUserExamDto is null)
                return null;
            else
                return _mapper.Map<ProctoringImageForUserExamVM>(proctoringImageForUserExamDto);
        }
